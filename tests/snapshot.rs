...
#[test]
fn report_shows_expected_panel_when_approved_png_missing_on_disk() {
    // The "frames committed, PNGs not" layout: check regenerates the
    // expected image in memory. The report must show THAT image, not a
    // "missing approval" caption pointing at a nonexistent disk path.
    let (_dir, st) = tmp_store("expectedpanel");
    let frame = frame_with("regen expected");
    st.check("home", &frame, &profile(), &VENDORED_FACES, 1.0)
        .unwrap();
    st.accept("home").unwrap();
    ...
    assert_eq!(
        outcome.status,
        Status::Matched,
        "pixels still gated against the regenerated expected"
    );
    // The defect: expected_png claimed a disk path that does not exist.
    assert!(outcome.expected_png.is_none());
    let bytes = outcome
        .expected_png_bytes
        .as_ref()
        .expect("regenerated bytes are kept on the outcome");
    let actual_bytes = std::fs::read(&outcome.actual_png).unwrap();
    assert_eq!(
        *bytes, actual_bytes,
