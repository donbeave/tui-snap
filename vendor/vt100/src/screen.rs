...
impl Screen {
    pub(crate) fn text(&mut self, c: char) {
        ...
        if self.mode(MODE_NO_AUTOWRAP) && width > 0 {
            // DECAWM disabled: overwrite the rightmost fitting cell, never wrap.
            // Keep the pending column for combining characters (width zero).
            let col = pos.col.min(size.cols - 1);
            if width > size.cols - col {
                // A wide glyph cannot fit; suppress it without shifting left.
                return;
            }
